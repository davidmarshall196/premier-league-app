...
def test_run_unit_tests(mocker):
    ...
    # When
    result = code_quality_functions.run_unit_tests(
        mock_test_file, mock_src_folder, mock_cov_folder)
...
def test_run_unit_tests_failure(mocker):
    ...
    # When
    result = code_quality_functions.run_unit_tests(
        mock_test_file, mock_src_folder)

    # Then
    assert result is None
    pytest.main.assert_called_once()
    print.assert_called_with("An error occurred while running tests: Tests failed. Please check.")
...
def test_run_code_quality_checks(returncode, stdout, stderr, expected):
    with patch('subprocess.run') as mock_subprocess_run:
        ...
        result = code_quality_functions.run_code_quality_checks(
            "src_folder")
