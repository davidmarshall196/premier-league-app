...
def add_new_data(
    ...
    - align_20 (bool): If True, aligns the number of rows to a multiple of 20 by trimming the dataset. Defaults to False.
    ...
    - In case of HTTPError, logs an error and returns the original full_data without changes.
...
def replace_team_names(
    ...
    """
    Replace team names in a specified column of a DataFrame according to a given mapping.

    Parameters:
    - input_data (pd.DataFrame): The DataFrame containing team names.
    - column (str): The name of the column in which team names are to be replaced.
    - replace_map (dict): A dictionary mapping old team names to new ones. Defaults to constants.TEAM_NAME_REPLACEMENTS.

    Returns:
    - pd.DataFrame: The DataFrame with team names replaced in the specified column.

    Notes:
    - This function is useful for standardizing team names across different datasets.
    """
...
def get_fixture_times(
    ...
    """
    Retrieve the upcoming fixture times for the current or next Premier League season.

    Returns:
    - Optional[pd.DataFrame]: A DataFrame containing the upcoming fixtures, or None if no fixtures are found.

    Notes:
    - Uses the `extract_season` function to determine the current season.
    - Attempts to retrieve fixtures for the next season if none are found for the current season.
    - Logs the process and any errors encountered.
    - Standardizes team names using the `replace_team_names` function.
    - Raises ValueError if no fixtures are found after attempting both the current and next season.
    """
    s = extract_season()
    s1 = s[0:2]
...
def extract_current_fixtures(
    transformed_data: pd.DataFrame
) -> Optional[pd.DataFrame]:
    """
    Extract upcoming fixtures from transformed data and match them with fixture times.

    Parameters:
    - transformed_data (pd.DataFrame): The transformed data containing historical match details.

    Returns:
    - Optional[pd.DataFrame]: A DataFrame containing the upcoming fixtures with additional data, or None if no fixtures are found.

    Notes:
    - Retrieves fixture times using the `get_fixture_times` function.
    - Merges the fixtures with transformed data on season, home team, and away team.
    - Asserts that the merged DataFrame contains data; raises AssertionError otherwise.
    - Processes fixture dates and times.
    - Filters fixtures based on the current date and returns the next 12 fixtures.
    """
    fix = get_fixture_times()
    if fix is not None:
        fix = fix.drop(['Result'], axis=1)
        transformed_data['season'] = transformed_data['season'].replace('season_', '')
        current_fixtures = pd.merge(transformed_data, fix, on=['season', 'HomeTeam', 'AwayTeam'], how='inner')
        assert current_fixtures.shape[0] > 0, "No matching fixtures found"
        current_fixtures['Fixture Date'] = pd.to_datetime(current_fixtures['Date'].str.slice(0,10), format='%d/%m/%Y')
        current_fixtures['Fixture Time'] = (
            current_fixtures['Date'].str.slice(11,13).astype(int) + 1).astype(str) + current_fixtures['Date'].str.slice(13,16)
        c_date = pd.to_datetime(datetime.datetime.now().strftime('%Y-%m-%d'), format='%Y-%m-%d')
        current_fixtures = current_fixtures.sort_values(by=['Fixture Date', 'Fixture Time'])
        current_fixtures = current_fixtures[current_fixtures['Fixture Date'] >= c_date]
        current_fixtures['Fixture'] = current_fixtures['HomeTeam'] + ' v ' + current_fixtures['AwayTeam']
...
def get_fixtures(full_data: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    Parameters:
    - full_data (pd.DataFrame): The existing dataset containing historical match data.
    ...
    Notes:
    - Uses the `extract_season` function to determine the current season.
    - Fetches fixture data for the current season and appends it to the existing dataset.
