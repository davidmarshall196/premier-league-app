...
def run_inference_pipeline() -> pd.DataFrame:
    ...
    It loads the data, preprocesses it, and predicts the outcomes
    ...
    # Load Expectations
    if constants.run_data_expectations:
        data_expectations = expectations_helpers.load_expectations(constants.exp_loc)

    # Load data
    if constants.initial_data_load:
        full_data = data_extraction.load_all_data(
            constants.seasons_list, constants.save_location, constants.columns_req
        )
    else:
    ...
    full_data = data_extraction.add_new_data(
        full_data, constants.columns_req, constants.training_data_location
    )
    ...
    # Validate
    if constants.run_data_expectations:
        expectations_helpers.validate_data(full_data, data_expectations)

    # Load preprocessor
    transformers = joblib.load(constants.transformer_path)

    # Transform data
    transformed_data = preprocessing.transform_data(full_data, transformers)
