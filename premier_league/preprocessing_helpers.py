...
def get_goals_scored(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_goals_conceded(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_gss(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_cuml_points(matchres: pd.DataFrame, games_1: int, r: int = 20) -> pd.DataFrame:
    """
    ...
    matchres_points.insert(column=0, loc=0, value=[0 * i for i in range(r)])
    return matchres_points


def get_matchres(
    playing_stat: pd.DataFrame, games_1: int, total_games: int
) -> pd.DataFrame:
    """
...
def get_agg_points(playing_stat: pd.DataFrame, r: int = 20) -> pd.DataFrame:
    """
...
def get_form(playing_stat: pd.DataFrame, num: int) -> pd.DataFrame:
    """
...
def add_form(playing_stat: pd.DataFrame, num: int) -> pd.DataFrame:
    """
...
def add_form_df(playing_statistics: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_mw(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def gss_all_seasons(playing_df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates the number of goals scored and conceded by each team in each
...
def points_all_seasons(playing_df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates the cumulative number of points earned by each team up to a
...
def form_all_seasons(playing_df: pd.DataFrame) -> pd.DataFrame:
    """
...
def mw_all_season(playing_df: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_form_points(string: str) -> int:
    """
...
def add_form_cols(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_3game_ws(string: str) -> int:
    """
...
def get_5game_ws(string: str) -> int:
    """
...
def get_3game_ls(string: str) -> int:
    """
...
def get_5game_ls(string: str) -> int:
    """
...
def add_win_streaks(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
    Adds columns to `playing_stat` indicating whether each team has won or lost three
    or five games in a row.

    Parameters:
    - playing_stat (pandas.DataFrame): A DataFrame containing the playing statistics
    for each team.

    Returns:
    - pandas.DataFrame: A copy of `playing_stat` with new columns indicating win/loss
    streaks for each team.
    """
...
def add_goal_difference(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def add_points_diff(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    Returns:
    - pd.DataFrame: The playing_stat DataFrame with added 'DiffPts' and
    'DiffFormPts' columns.
    """
...
def league_position(playing_stat: pd.DataFrame, season: int, mw: int) -> pd.DataFrame:
    """
    ...
    Returns:
    - pd.DataFrame: The league position DataFrame with columns 'season',
    'MW', 'Team', 'LOC' (Home or Away), and 'LeaguePosition'.
    """
    league_pos = playing_stat[
        (playing_stat["season"] == season) & (playing_stat["MW"] == mw)
    ]
    ...
    league_pos_df = league_pos_df.sort_values(
        by=["Points", "GD", "Team"]
    ).drop_duplicates(subset="Team")
    league_pos_df["LeaguePosition"] = (
        league_pos_df["Points"].rank(method="first", ascending=False).astype(int)
    )
    league_pos_df = league_pos_df[["season", "MW", "Team", "LOC", "LeaguePosition"]]

    home_df = league_pos_df[league_pos_df["LOC"] == "Home"].drop("LOC", axis=1)
    away_df = league_pos_df[league_pos_df["LOC"] == "Away"].drop("LOC", axis=1)
    key = ["season", "MW"]
    ...
    league_pos = (
        ...
        .drop("Team", axis=1)
        .rename(columns={"LeaguePosition": "HomeLeaguePosition"})
    )
    league_pos = (
        pd.merge(
            league_pos,
            away_df,
            left_on=key + ["AwayTeam"],
            right_on=key + ["Team"],
            how="left",
        )
        .drop("Team", axis=1)
        .rename(columns={"LeaguePosition": "AwayLeaguePosition"})
    )
    return league_pos


def league_pos_all_seasons(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
    Computes the league position for all seasons and matchweeks in the playing_stat
    DataFrame.
    ...
    Returns:
    - pd.DataFrame: The final league position DataFrame with columns 'season',
    'MW', 'Team', 'LOC' (Home or Away), 'HomeLeaguePosition', 'AwayLeaguePosition',
    ...
    league_pos_final["AwayLeaguePosition"] = (
        league_pos_final.groupby(["season", "AwayTeam"])["AwayLeaguePosition"]
        .ffill()
        .fillna(0)
        .astype(int)
    )
    league_pos_final["LeaguePositionDiff"] = (
        league_pos_final["HomeLeaguePosition"] - league_pos_final["AwayLeaguePosition"]
    )
    return league_pos_final


def scale_points_mw(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def convert_to_date(date: str) -> pd.Timestamp:
    """
...
def add_date_features(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    playing_stat["pd_date"] = playing_stat["Date"].apply(convert_to_date)
    playing_stat["MatchDayDay"] = playing_stat["pd_date"].dt.strftime("%A")
    playing_stat["MatchDayMonth"] = playing_stat["pd_date"].dt.strftime("%B")
    playing_stat["MatchDayDate"] = playing_stat["pd_date"].dt.strftime("%d")
    return playing_stat


def convert_to_string(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def drop_cols(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    difference in points and form points.
    """
    drop_cols = ["Date", "FTHG", "FTAG", "HTFormPtsStr", "ATFormPtsStr", "pd_date"]
    for c in drop_cols:
        try:
...
def convert_categorical(playing_stat: pd.DataFrame, target: str) -> pd.DataFrame:
    """
    ...
    """
    cat_cols = playing_stat.select_dtypes("object").columns.to_list()
    if target == "FTR":
    ...
    if target == "FTHG" or target == "FTAG":
        enc = TargetEncoder(cols=cat_cols, min_samples_leaf=20, smoothing=10)
        enc = enc.fit_transform(
            X=playing_stat.drop(target, axis=1), y=playing_stat[target]
        )
        numeric_dataset = enc.transform(playing_stat.drop(target, axis=1))
        return numeric_dataset
    else:
        return playing_stat


def only_hw(string: str) -> int:
    """
...
def add_target(playing_stat: pd.DataFrame) -> pd.DataFrame:
    """
...
def run_pipeline(playing_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    playing_stat = merge_data(gs_df_full, points_df_full, form_df_full, match_week_full)
    ...
    playing_stat["MatchDayDate"] = playing_stat["MatchDayDate"].astype(int)
    playing_stat["season"] = playing_stat["season"].astype(str).astype("category")
...
def add_season(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["season_1"] = (
        data["last_year"].astype(str) + "-" + data["year"].str.slice(2, 4)
    )
    data["season_2"] = (
        data["year"].astype(str) + "-" + data["next_year"].astype(str).str.slice(2, 4)
    )
    data["season"] = np.where(data["month_num"] < 8, data["season_1"], data["season_2"])
    data = data.drop(
        [
            "pd_date",
            "year",
